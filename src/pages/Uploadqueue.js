...
const Uploadqueue = () => {
  ...
  const redirectCompletedrecording = () => {
    navigate("/uploadqueue/completedrecording");
  };
...
const handleFilterChange = (value) => {
  setSearchFilter(value);   // update the active filter
  setSearchTerm("");        // optional: clear the previous search term
  ...
  const handleKeyboardPress = (key) => {
    ...
    if (key === "Back") {
      ...
      if (key === "Enter") {
    // trigger filter by current activeField & searchTerm
    setCurrentPage(1);  
    setShowKeyboard(false);
    return;
  }
